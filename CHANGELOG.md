...
- `list_databases` and `list_tables` are now registered as async tools that run their ClickHouse work on the query worker pool, so metadata calls no longer block the event loop. Tool names, arguments, and JSON responses are unchanged. The exported Python helpers keep their synchronous signatures.
