...
- Tool annotations (`read_only_hint`, `destructive_hint`, `idempotent_hint`, `open_world_hint`) are part of the observable contract. The `run_query` annotations are computed at registration from `CLICKHOUSE_ALLOW_WRITE_ACCESS` and `CLICKHOUSE_ALLOW_DROP` and must keep tracking those gates; the metadata tools are read-only; the chDB tool is read-only for the in-memory `CHDB_DATA_PATH` and writable and destructive for a filesystem path. The `run_query` and chDB descriptions are computed at registration from the same configuration and state the mode. Assert all of this through `Client.list_tools()` when changing registration.
