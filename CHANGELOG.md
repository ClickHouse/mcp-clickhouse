...
- **Breaking for custom middleware:** FastMCP context state is asynchronous. Middleware that sets `clickhouse_client_config_overrides` must now `await ctx.set_state(...)`. Overrides remain request-scoped; the sessionless protocol has no cross-request session state.
