...
from mcp_clickhouse.mcp_env import get_config, get_chdb_config, get_mcp_config, get_pgvector_config
...
def create_pgvector_client():
    """Create a PostgreSQL client connection with pgvector support."""
    try:
        import psycopg2
    ...
    logger.info(
        ...
        f"as {client_config['user']} (database={client_config['database']}, "
...
def execute_pgvector_query(query: str):
    """Execute a query on PostgreSQL with pgvector."""
    conn = None
    cursor = None
    try:
...
def search_similar_vectors(
    ...
    try:
        ...
        raise ToolError(f"Failed to perform vector search: {str(e)}")


def pgvector_initial_prompt() -> str:
    """This prompt helps users understand how to interact and perform operations with pgvector"""
    return PGVECTOR_PROMPT


# Register pgvector tools based on configuration
if os.getenv("PGVECTOR_ENABLED", "false").lower() == "true":
    ...
    pgvector_prompt = Prompt.from_function(
        ...
        description="This prompt helps users understand how to interact and perform operations with pgvector",
    )
