...
def reset_advisory_flag():
    ...
    mcp_server._clickhouse_clients.grants_advisory_done = False
    yield
    mcp_server._clickhouse_clients._clear_client_cache()
    mcp_server._clickhouse_clients.grants_advisory_done = False
...
def test_warns_on_dangerous_privilege(caplog, grant, privilege):
    ...
    with caplog.at_level(logging.WARNING, logger=LOGGER_NAME):
        _clickhouse_clients._warn_if_overprivileged(client)
...
def test_advisory_runs_in_write_mode_without_drop(caplog):
    ...
    with patch("mcp_clickhouse.clients.get_config", return_value=config):
        with patch("mcp_clickhouse.clients.clickhouse_connect.get_client", return_value=client):
            with caplog.at_level(logging.WARNING, logger=LOGGER_NAME):
...
def test_advisory_skipped_for_request_override_client(caplog):
    ...
    with patch("mcp_clickhouse.clients.get_config", return_value=config):
        with patch("mcp_clickhouse.clients.clickhouse_connect.get_client", return_value=client):
            ...
            assert mcp_server._clickhouse_clients.grants_advisory_done is False
